...
public:
   ...
   */
  WAVAccumulator(unsigned uMaxInputBufferSize,
...
private:
  const double
      m_dContinuityThresholdFactor; ///< Double which defines max jitter before
  unsigned m_dAccumulatePeriod;     ///< Maximum number of seconds with which a
                                    ///< module will make a recording
                                    ///< signal considered discontinuous
  std::map<std::vector<uint8_t>, std::shared_ptr<BaseChunk>>
      m_mAccumulatedWAVChunks; ///< Map of accumulated WAV chunks as a function
                               ///< of string mac addr
   ...
   */
  void Accumuluate(const std::shared_ptr<WAVChunk> &pAccumulatedWAVChunk,
                   const std::shared_ptr<WAVChunk> &pCurrentWAVChunk);

  /*
   * @brief Verifies that current chunk is continuous with currently accumulated
   * data
   * @param[in] pCurrentWAVChunk Pointer to current time chunk
   * @param[in] pAccumulatedWAVChunk  Pointer to accumulated time chunk
   */
  bool VerifyTimeContinuity(std::shared_ptr<WAVChunk> pCurrentWAVChunk,
                            std::shared_ptr<WAVChunk> pAccumulatedWAVChunk);

  /*
   * @brief Checks if the current WAV chunk (recording) meets time threshold and
   * can therefore be passed on
  ...
  /*
   * @brief Checks if the end device recording mode has been changed since last
   * accumulation
   * @param[in] pAccumulatedWAVChunk pAccumulatedWAVChunk pointer to accumulated
   * WAV chunk data
   * @param[in] pCurrentWAVChunk pointer to current WAV chunk
   */
  bool WAVHeaderChanged(std::shared_ptr<WAVChunk> pAccumulatedWAVChunk,
