...
SoundCardInterfaceModule::SoundCardInterfaceModule(
    unsigned uBufferSize, nlohmann::json_abi_v3_11_2::json jsonConfig)
    : BaseModule(uBufferSize),
      m_uNumChannels(CheckAndThrowJSON<double>(jsonConfig, "NumberOfChannels")),
  ...
  std::string strInfo =
      ...
      "=========\n" + +"SourceIdentifier [ " +
      std::string(m_vu8SourceIdentifier.begin(), m_vu8SourceIdentifier.end()) +
...
bool SoundCardInterfaceModule::UpdatePCMSamples() {
  ...
  if ((err = snd_pcm_readi(m_capture_handle, m_pvcAudioData->data(),
                           iBufferFrames)) != iBufferFrames) {
    fprintf(stderr,
            "Read from audio interface failed (%d) (possible overrun): %s\n",
  ...
  for (unsigned uCurrentSampleIndex = 0; uCurrentSampleIndex < m_dChunkSize;
    ...
    for (unsigned uCurrentChannelIndex = 0;
         uCurrentChannelIndex < m_uNumChannels; uCurrentChannelIndex++) {
      m_pTimeChunk
          ->m_vvi16TimeChunks[uCurrentChannelIndex][uCurrentSampleIndex] =
          samples[uCurrentSampleIndex * m_uNumChannels + uCurrentChannelIndex];
    }
  }
...
void SoundCardInterfaceModule::InitALSA() {
  ...
  if ((err = snd_pcm_open(&m_capture_handle, m_pDevice, SND_PCM_STREAM_CAPTURE,
                          0)) < 0) {
    PLOG_ERROR << "cannot open audio " + std::string(m_pDevice) + " default ("
               << snd_strerror(err) << ")";
    Cleanup();
    exit(1);
  }
  ...
  if ((err = snd_pcm_hw_params_malloc(&m_hw_params)) < 0) {
    PLOG_ERROR << "cannot allocate hardware parameter structure ("
               << snd_strerror(err) << ")";
    Cleanup();
    exit(1);
  }
  ...
  if ((err = snd_pcm_hw_params_any(m_capture_handle, m_hw_params)) < 0) {
    PLOG_ERROR << "cannot initialize hardware parameter structure ("
               << snd_strerror(err) << ")";
    Cleanup();
    exit(1);
  }
  ...
  if ((err = snd_pcm_hw_params_set_access(m_capture_handle, m_hw_params,
  ...
  if ((err = snd_pcm_hw_params_set_format(m_capture_handle, m_hw_params,
                                          m_format)) < 0) {
  ...
  if ((err = snd_pcm_hw_params_set_rate_near(m_capture_handle, m_hw_params,
                                             &uSampleRate, 0)) < 0) {
  ...
  if ((err = snd_pcm_hw_params_set_channels(m_capture_handle, m_hw_params,
                                            m_uNumChannels)) < 0) {
  ...
  snd_pcm_uframes_t buffer_size =
      period_size * 6; // Adjust this value as needed

  if ((err = snd_pcm_hw_params_set_buffer_size_near(
           m_capture_handle, m_hw_params, &buffer_size)) < 0) {
  ...
  if ((err = snd_pcm_hw_params_set_period_size_near(
           m_capture_handle, m_hw_params, &period_size, 0)) < 0) {
  ...
  if ((err = snd_pcm_prepare(m_capture_handle)) < 0) {
    PLOG_ERROR << "cannot prepare audio interface for use ("
               << snd_strerror(err) << ")";
    Cleanup();
    exit(1);
  }
  ...
  m_pTimeChunk = std::make_shared<TimeChunk>(m_dChunkSize, m_dSampleRate, 0, 12,
                                             sizeof(float), 1);
  m_pvcAudioData = std::make_shared<std::vector<char>>(
      m_dChunkSize * m_uNumChannels * snd_pcm_format_width(m_format) / 8);
}
