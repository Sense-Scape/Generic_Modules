...
HTTPPostModule::HTTPPostModule(unsigned uBufferSize, std::string strEndpoint, const nlohmann::json_abi_v3_11_2::json& jsonConfig)
...
bool HTTPPostModule::SendBody(std::shared_ptr<BaseChunk> pBaseChunk)
{
    ...
    if (res != CURLE_OK)
    {
        PLOG_ERROR << "HTTP Error: " << curl_easy_strerror(res);
        return false;
    }
    ...
    {
        PLOG_ERROR << "HTTP Code: " << httpCode <<  " with response " << responseBody << " from " <<  m_strEndpoint; 
