...
LinuxMultiClientTCPRxModule::LinuxMultiClientTCPRxModule(
    unsigned uMaxInputBufferSize, nlohmann::json_abi_v3_11_2::json jsonConfig)
    : BaseModule(uMaxInputBufferSize),
      m_strIPAddress(CheckAndThrowJSON<std::string>(jsonConfig, "IP")),
      m_u16TCPPort(CheckAndThrowJSON<uint16_t>(jsonConfig, "Port")),
      m_iDatagramSize(512), m_u16LifeTimeConnectionCount(0) {}

LinuxMultiClientTCPRxModule::~LinuxMultiClientTCPRxModule() {}

void LinuxMultiClientTCPRxModule::Process(
    std::shared_ptr<BaseChunk> pBaseChunk) {
  while (!m_bShutDown) {
    // Connect to the allocation port and start listening for client to
    // connections
...
void LinuxMultiClientTCPRxModule::ConnectTCPSocket(int &sock,
                                                   uint16_t u16TCPPort) {
  ...
  // {
  //     std::string strError = std::string(__FUNCTION__) + " ioctlsocket
  ...
  if (listen(sock, SOMAXCONN) < 0) {
    std::string strError = std::string(__FUNCTION__) +
                           ": Error listening on server socket. Error code: " +
                           std::to_string(errno) + " ";
    PLOG_ERROR << strError;
  ...
  std::string strInfo =
      ...
      ": Socket binding complete, listening on IP: " + m_strIPAddress +
      " and port " + std::to_string(u16TCPPort) + " ";
  PLOG_INFO << strInfo;
}

void LinuxMultiClientTCPRxModule::AllocateAndStartClientProcess(
    int &AllocatingServerSocket) {
  {
    std::string strInfo =
        std::string(__FUNCTION__) + ": Waiting for client connection requests";
    PLOG_INFO << strInfo;
  }
  ...
  if (PortNumberAllcationSocket < 0) {
    std::string strWarning =
        std::string(__FUNCTION__) +
        ": Error accepting client connection. Error code: " +
        std::to_string(errno) + "";
  ...
  std::string strInfo = std::string(__FUNCTION__) +
                        ": Accepted client connection. Client socket: " +
                        std::to_string(PortNumberAllcationSocket) + "";
  PLOG_INFO << strInfo;

  // Increment and define port we can allocate to new client to not have
  // port clash
  ...
  {
    std::string strInfo = std::string(__FUNCTION__) +
                          ": Begining client port allocation to port " +
                          std::to_string(u16AllocatedPortNumber);
    PLOG_INFO << strInfo;
  }
  ...
  // And once the thread is operation we transmit the port the thread uses
  // to the client
  ...
  int bytes_sent =
      send(PortNumberAllcationSocket, &vcData[0], vcData.size(), 0);
  if (bytes_sent < 0) {
    // The client should close the connection once it has figured out the
    // port it should use
    throw;
  }
...
void LinuxMultiClientTCPRxModule::StartClientThread(
  ...
  if (getpeername(clientSocket, (sockaddr *)&clientAddr, &clientAddrLen) == 0) {
    ...
    std::string strInfo =
        std::string(__FUNCTION__) + ": Client connected from IP: " + clientIP;
    PLOG_INFO << strInfo;
  } else {
  ...
  {
    std::string strInfo =
        std::string(__FUNCTION__) + ": Starting client thread ";
    PLOG_INFO << strInfo;
  }
  ...
  setsockopt(clientSocket, SOL_SOCKET, SO_RCVTIMEO, (const char *)&recvTimeout,
             sizeof(recvTimeout));
  ...
  while (!m_bShutDown) {
    ...
    if (num_ready < 0) {
      std::string strWarning =
          std::string(__FUNCTION__) +
          ": Failed to wait for data on socket for client " + clientIP +
          " with error: " + std::to_string(errno);
      PLOG_WARNING << strWarning;
      break; // Exit the loop on error
    } else if (num_ready == 0) {
      std::string strWarning = std::string(__FUNCTION__) +
                               ": Timeout occured after 10s for client " +
                               clientIP;
      PLOG_WARNING << strWarning;
      break; // Exit the loop on error
    }
    ...
    if (FD_ISSET(clientSocket, &readfds)) {
      ...
      auto vcCompleteClassByteVector = std::vector<char>(
          ...
          vcAccumulatedBytes.begin() + sizeof(u16SessionTransmissionSize) +
              u16SessionTransmissionSize);

      // And then create a vector of the remaining bytes to carry on
      // processing with
...
void LinuxMultiClientTCPRxModule::CloseTCPSocket(int socket) { close(socket); }
...
bool LinuxMultiClientTCPRxModule::CheckForSocketReadErrors(
    ssize_t stReportedSocketDataLength, size_t stActualDataLength) {
  // Check for timeout
  if (stReportedSocketDataLength == -1 && errno == EAGAIN) {
    std::string strWarning =
        std::string(__FUNCTION__) +
        ": recv() timed out  or errored for client (error: " +
        std::to_string(errno) + ") " + m_strIPAddress;
    PLOG_WARNING << strWarning;
    return true;
  }
    ...
    std::string strInfo = std::string(__FUNCTION__) + ": client " +
                          m_strIPAddress +
                          " closed connection closed, shutting down thread";
    PLOG_INFO << strInfo;
    return true;
  }
