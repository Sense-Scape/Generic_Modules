...
SimulatorModule::SimulatorModule(unsigned uBufferSize, nlohmann::json_abi_v3_11_2::json jsonConfig) : 
...
void SimulatorModule::ConfigureModuleJSON(nlohmann::json_abi_v3_11_2::json jsonConfig)
{
    m_dChunkSize = 512;
    ...
    std::transform(m_vfChannelPhases_rad.begin(),m_vfChannelPhases_rad.end(),m_vfChannelPhases_rad.begin(),
...
void SimulatorModule::GenerateGaussianNoise()
{
    for (unsigned uCurrentSampleIndex = 0; uCurrentSampleIndex < m_dChunkSize; uCurrentSampleIndex++)
    {
        for (unsigned uChannel = 0; uChannel < m_uNumChannels; uChannel++)
        {
            ...
            m_pTimeChunk->m_vvi16TimeChunks[uChannel][uCurrentSampleIndex] = m_dist(m_generator);
            m_count++;
        }   
    }
}
...
void SimulatorModule::SimulateADCSamples()
{
    if (m_strADCMode == "Sinusoid")
        ...
        throw std::runtime_error("ADC Mode incorrectly specified as: " + m_strADCMode);
