...
void LinuxMultiClientTCPTxModule::ConnectTCPSocket(int &sock,
                                                   uint16_t u16TCPPort) {
  ...
  std::string strInfo =
      std::string(__FUNCTION__) + ": Connecting to Server at ip " +
      m_sDestinationIPAddress + " on port " + std::to_string(u16TCPPort) + "";
  PLOG_INFO << strInfo;
  ...
  if (iConnectResult == 0) {
    std::string strInfo =
        std::string(__FUNCTION__) + ": Connected to server at ip " +
        m_sDestinationIPAddress + " on port " + std::to_string(u16TCPPort) + "";
    PLOG_INFO << strInfo;
    m_bTCPConnected = true;
  } else {
    std::string strWarning =
        std::string(__FUNCTION__) + ": Failed to connect to the server(" +
        m_sDestinationIPAddress + ") on port " + std::to_string(u16TCPPort) +
...
uint16_t
LinuxMultiClientTCPTxModule::WaitForReturnedPortAllocation(int &WinSocket) {
  ...
  if (num_ready < 0) {
    std::string strWarning =
        std::string(__FUNCTION__) + ": Failed to wait for data on socket ";
    PLOG_WARNING << strWarning;
  }
  ...
  if (FD_ISSET(WinSocket, &readfds)) {
    ...
    while (vcAccumulatedBytes.size() < sizeof(uint16_t)) {
      ...
      int16_t i16ReceivedDataLength =
          recv(WinSocket, &vcByteData[0], sizeof(uint16_t), 0);

      bReadError =
          CheckForSocketReadErrors(i16ReceivedDataLength, vcByteData.size());
      if (bReadError)
        break;
  ...
  LOG_WARNING << "Client allocated port " +
                     std::to_string(u16AllocatedPortNumber);

  return u16AllocatedPortNumber;
}

void LinuxMultiClientTCPTxModule::Process(
  ...
  while (!m_bShutDown) {
    if (!m_bTCPConnected) {
      ...
      auto u16AllocatedPortNumber =
          WaitForReturnedPortAllocation(AllocatingServerSocket);
      ...
      std::thread clientThread(
          [this, &AllocatedServerSocket, u16AllocatedPortNumber] {
...
void LinuxMultiClientTCPTxModule::RunClientThread(
    int &clientSocket, uint16_t u16AllocatedPortNumber) {
  while (!m_bShutDown) {
    try {
...
bool LinuxMultiClientTCPTxModule::CheckForSocketReadErrors(
  ...
  if (stReportedSocketDataLength == -1 && errno == EAGAIN) {
    std::string strWarning =
        ...
        std::to_string(errno) + ") " + m_sDestinationIPAddress;
    PLOG_WARNING << strWarning;
    return true;
  }
