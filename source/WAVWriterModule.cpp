...
void WAVWriterModule::WriteWAVFile(std::shared_ptr<BaseChunk> pBaseChunk) {
  ...
  if (!outFile) {
    PLOG_ERROR << "Failed to open file: " << sFileName << std::endl;
  ...
  pWAVChunk->m_sWAVHeader.bytesPerSec = pWAVChunk->m_sWAVHeader.SamplesPerSec *
                                        pWAVChunk->m_sWAVHeader.NumOfChan *
...
void WAVWriterModule::Process_WAVChunk(std::shared_ptr<BaseChunk> pBaseChunk) {
  if (IsEnoughFileSystemSpace())
    ...
    std::string strWarning = std::string(__FUNCTION__) +
                             "Not enough space to write wav file, data lost";
...
void WAVWriterModule::CreateFilePath() {
  if (!std::filesystem::exists(m_sFileWritePath)) {
    ...
    std::string strInfo = std::string(__FUNCTION__) +
                          ": Folder created successfully - " + m_sFileWritePath;
    PLOG_INFO << strInfo;
  }
}
